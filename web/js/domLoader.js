...
function createScoreBarSVG() {
	...
	const backdropBlack = document.createElementNS(svgNS, "rect");
	backdropBlack.setAttributeNS(null, "x" , 0);
	backdropBlack.setAttributeNS(null, "y" , 0);
	backdropBlack.setAttributeNS(null, "width" , 0);
	backdropBlack.setAttributeNS(null, "height", height);
	backdropBlack.setAttributeNS(null, "fill", "rgb(60, 60, 60)")
	backdropBlack.setAttributeNS(null, "id", `black-points-back`);
	blackGroup.appendChild(backdropBlack);

	// Create a group for white points
	const whiteGroup = document.createElementNS(svgNS, "g");
	svg.appendChild(whiteGroup);

	const backdropwhite = document.createElementNS(svgNS, "rect");
	backdropwhite.setAttributeNS(null, "x" , 0);
	backdropwhite.setAttributeNS(null, "y" , 0);
	backdropwhite.setAttributeNS(null, "width" , 0);
	backdropwhite.setAttributeNS(null, "height", height);
	backdropwhite.setAttributeNS(null, "fill", "rgb(175, 175, 175)");
	backdropwhite.setAttributeNS(null, "id", `white-points-back`);
	whiteGroup.appendChild(backdropwhite);

	// Generate black points from left to right (1 to 0.1)
	for (let i = totalBoxes; i >= 1; i--) {
		...
		box.setAttributeNS(null, "id", `black-points-${i}`);
	...
	for (let i = 0; i < totalBoxes; i++) {
		...
		let colorValue = 245 - 10 * i; // 238 corresponds to #EEEEEE, incrementing towards 255 (#FFFFFF)
