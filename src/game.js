...
class GameEntity {
    ...
    calculateHumanWinRate() {
        ...
        function QuentinWinrateFunction(x, n_move, L) {
            ...
            const middleDenominator = Math.pow((1 + Math.pow(Math.abs(g(n_move, L) * x), k(n_move))), 1 / k(n_move));
    ...
    determineLead() {
        const current = Math.sign(this.ai.score) > 0 ? this.current.player : (this.current.player === 'W' ? 'B' : 'W');
